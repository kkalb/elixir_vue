defmodule ElixirVueWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: ElixirVueWeb.Gettext
  ...
  def input(%{type: "date"} = assigns) do
    ...
    classes =
      if Map.has_key?(assigns.rest, :readonly),
        do: "hover:cursor-default font-normal disabled",
        else: ""
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ElixirVueWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ElixirVueWeb.Gettext, "errors", msg, opts)
    end
  end
    ...
    <footer class="fixed w-full h-8 md:h-8 bottom-0 left-0 right-0 border-t-0 md:border-t md:bg-cgray/25 border-corange/20 shadow text-cwhite">
      <div class="flex flex-row items-center justify-between md:ml-24 md:mr-10 mr-0 ml-2">
        <p class="flex justify-center">
          ©Kevin Kalb, All rights reserved. Version {Application.spec(:elixir_vue)[:vsn]}
        </p>
        <ul class="flex flex-row gap-x-0 md:gap-x-4">
          <li class="">
            <a rel="noopener nofollow" target="_blank" href="https://github.com/elixir_vue">
              <svg
                width="36"
                height="36"
                viewBox="0 0 24 24"
